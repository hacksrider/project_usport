...
export async function POST(req: Request) {
  try {
    ...
    if (selector === "insert") {
      for (let i = 0; i < getData.length; i++) {
        ...
        if (
          ...
          throw new Error(
            `ข้อมูลไม่ครบถ้วนสำหรับ period_ID: ${item.period_ID}`
          );
        }
      ...
      throw new Error("กรุณาระบุ case_type เป็น insert");
    }
    return new Response(
      JSON.stringify({ message: "Prices inserted successfully", data: result }),
      {
        status: 200,
    ...
    console.log(error);
    return new Response(JSON.stringify({ error: error || "เกิดข้อผิดพลาด" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
